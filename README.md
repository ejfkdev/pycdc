...
|---|---|---|---|
| 2.7 | 10/13 | 3.9 | 11/16 |
| 3.3 | 11/13 | 3.10 | 12/17 |
| 3.5 | 11/13 | 3.11 | 13/17 |
| 3.6 | 11/14 | 3.12 | 15/17 |
| 3.7 | 11/14 | 3.13 | 14/17 |
| 3.8 | 12/16 | 3.14 | 10/17 |

（带 MIN/MAX_VERSION 门控的用例只在适用版本运行；未通过的用例集中在
生成器/async-with 超时、match、部分 slice 与短路副作用场景。）

## 已知限制

- try/except 的 3.8–3.10 SETUP_* 时代结构与 bare `except:` 还原不完整；3.11+ 基于异常表重建（含多 handler、嵌套 try、finally、as-cleanup、下沉流），主体正确。
