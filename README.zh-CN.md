...
| 项目（tag） | 模块数 | 串行 `-j 1` | 并行（默认） | 产物可重编译 |
|---|---|---|---|---|
...
| **合计** | **6411** | **6.61 s** | **2.14 s** | **6394/6411（99.7%）** |

每个项目一个 pycdc 进程：串行约 1.0 ms/模块，整批 6.4k 模块峰值
RSS ≈102 MB。剩余 17 个模块集中在「取反的多链条件」（`not` 下的
`is`/比较链 guard）与少量 lambda/推导式形态——输出标记为不完整，
不崩溃。原始数据见 [`benchmarks/realworld.json`](benchmarks/realworld.json)，
