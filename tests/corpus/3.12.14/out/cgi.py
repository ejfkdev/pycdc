...
def parse_multipart(fp, pdict, encoding='utf-8', errors='replace', separator='&'):
    ...
    try:
        ...
        pass
    fs = FieldStorage(fp, headers, encoding, errors, {'REQUEST_METHOD': 'POST'}, separator)
...
def _parseparam(s):
    while s[:1] == ';':
        ...
        if end > 0 and (s.count('"', 0, end) - s.count('\\"', 0, end)) % 2 and end > 0 and (s.count('"', 0, end) - s.count('\\"', 0, end)) % 2:
            end = s.find(';', end + 1)
            continue
        if end < 0:
            end = len(s)
...
class FieldStorage:
    ...
    def read_multi(self, environ, keep_blank_values, strict_parsing):
        ...
        if first_line.strip() != b'--' + self.innerboundary and first_line and first_line.strip() != b'--' + self.innerboundary and first_line:
            first_line = self.fp.readline()
            self.bytes_read += len(first_line)
    ...
    def _FieldStorage__write(self, line):
        if not self._FieldStorage__file is None:
