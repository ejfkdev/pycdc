    ...
    if workers != 1:
        if not ProcessPoolExecutor is None:
            ...
            with ProcessPoolExecutor(max_workers=workers, mp_context=mp_context) as executor:
                results = executor.map(partial(compile_file, ddir, force, rx, quiet, legacy, optimize, invalidation_mode, stripdir, prependdir, limit_sl_dest, hardlink_dupes), files)
            ...
            if compile_dests:
                for dest in compile_dests:
                    if os.path.isfile(dest):
                        if compile_file(dest, args.ddir, args.force, args.rx, args.quiet, args.legacy, invalidation_mode, args.stripdir, args.prependdir, args.opt_levels, args.limit_sl_dest, args.hardlink_dupes):
                            continue
                            try:
                                success = False
                                continue
