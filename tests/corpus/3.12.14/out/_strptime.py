...
class LocaleTime(object):
    ...
    def _LocaleTime__calc_date_time(self):
        ...
        replacement_pairs = [('1999', '%Y'), ('99', '%y'), ('22', '%H'), ('44', '%M'), ('55', '%S'), ('76', '%j'), ('17', '%d'), ('03', '%m'), ('3', '%m'), ('2', '%w'), ('10', '%I'), ('١٩٩٩', '%Y'), ('٩٩', '%Oy'), ('٢٢', '%OH'), ('٤٤', '%OM'), ('٥٥', '%OS'), ('١٧', '%Od'), ('٠٣', '%Om'), ('٣', '%Om'), ('٢', '%Ow'), ('١٠', '%OI')]
        date_time = []
...
class TimeRE(dict):
    ...
    def __init__(self, locale_time=None):
        ...
        base = super()
        mapping = {'d': '(?P<d>3[0-1]|[1-2]\\d|0[1-9]|[1-9]| [1-9])', 'f': '(?P<f>[0-9]{1,6})', 'H': '(?P<H>2[0-3]|[0-1]\\d|\\d)', 'I': '(?P<I>1[0-2]|0[1-9]|[1-9]| [1-9])', 'G': '(?P<G>\\d\\d\\d\\d)', 'j': '(?P<j>36[0-6]|3[0-5]\\d|[1-2]\\d\\d|0[1-9]\\d|00[1-9]|[1-9]\\d|0[1-9]|[1-9])', 'm': '(?P<m>1[0-2]|0[1-9]|[1-9])', 'M': '(?P<M>[0-5]\\d|\\d)', 'S': '(?P<S>6[0-1]|[0-5]\\d|\\d)', 'U': '(?P<U>5[0-3]|[0-4]\\d|\\d)', 'w': '(?P<w>[0-6])', 'u': '(?P<u>[1-7])', 'V': '(?P<V>5[0-3]|0[1-9]|[1-4]\\d|\\d)', 'y': '(?P<y>\\d\\d)', 'Y': '(?P<Y>\\d\\d\\d\\d)', 'z': '(?P<z>[+-]\\d\\d:?[0-5]\\d(:?[0-5]\\d(\\.\\d{1,6})?)?|(?-i:Z))', 'A': self._TimeRE__seqToRE(self.locale_time.f_weekday, 'A'), 'a': self._TimeRE__seqToRE(self.locale_time.a_weekday, 'a'), 'B': self._TimeRE__seqToRE(self.locale_time.f_month[1:], 'B'), 'b': self._TimeRE__seqToRE(self.locale_time.a_month[1:], 'b'), 'p': self._TimeRE__seqToRE(self.locale_time.am_pm, 'p'), 'Z': self._TimeRE__seqToRE((tz for tz_names in self.locale_time.timezone for tz in tz_names), 'Z'), '%': '%'}
        for d in 'dmyHIMS':
    ...
    with _cache_lock:
        ...
        if not iso_year is None:
            ...
            if not iso_week is None:
                if not weekday is not None or iso_week is None:
