...
'''
    ...
    def __init__(self):
        ...
        self._LocaleTime__calc_alt_digits()
    ...
    def _LocaleTime__calc_date_time(self):
        ...
        for directive in ('%c', '%x', '%X', '%r'):
            ...
            lst, fmt = self._LocaleTime__find_weekday_format(directive)
            if lst:
                current_format = current_format.replace(lst[2], fmt, 1)
            lst, fmt = self._LocaleTime__find_month_format(directive)
            if lst:
...
class TimeRE(dict):
    ...
    def __init__(self, locale_time=None):
        ...
        base = super()
        mapping = {'d': '(?P<d>3[0-1]|[1-2]\\d|0[1-9]|[1-9]| [1-9])', 'f': '(?P<f>[0-9]{1,6})', 'H': '(?P<H>2[0-3]|[0-1]\\d|\\d| \\d)', 'k': '(?P<H>2[0-3]|[0-1]\\d|\\d| \\d)', 'I': '(?P<I>1[0-2]|0[1-9]|[1-9]| [1-9])', 'l': '(?P<I>1[0-2]|0[1-9]|[1-9]| [1-9])', 'G': '(?P<G>\\d\\d\\d\\d)', 'j': '(?P<j>36[0-6]|3[0-5]\\d|[1-2]\\d\\d|0[1-9]\\d|00[1-9]|[1-9]\\d|0[1-9]|[1-9])', 'm': '(?P<m>1[0-2]|0[1-9]|[1-9])', 'M': '(?P<M>[0-5]\\d|\\d)', 'S': '(?P<S>6[0-1]|[0-5]\\d|\\d)', 'U': '(?P<U>5[0-3]|[0-4]\\d|\\d)', 'w': '(?P<w>[0-6])', 'u': '(?P<u>[1-7])', 'V': '(?P<V>5[0-3]|0[1-9]|[1-4]\\d|\\d)', 'y': '(?P<y>\\d\\d)', 'Y': '(?P<Y>\\d\\d\\d\\d)', 'z': '(?P<z>[+-]\\d\\d:?[0-5]\\d(:?[0-5]\\d(\\.\\d{1,6})?)?|(?-i:Z))', 'A': self._TimeRE__seqToRE(self.locale_time.f_weekday, 'A'), 'a': self._TimeRE__seqToRE(self.locale_time.a_weekday, 'a'), 'B': self._TimeRE__seqToRE(_fixmonths(self.locale_time.f_month[1:]), 'B'), 'b': self._TimeRE__seqToRE(_fixmonths(self.locale_time.a_month[1:]), 'b'), 'p': self._TimeRE__seqToRE(self.locale_time.am_pm, 'p'), 'Z': self._TimeRE__seqToRE((tz for tz_names in self.locale_time.timezone for tz in tz_names), 'Z'), '%': '%'}
        if not self.locale_time.LC_alt_digits is not None:
            for d in 'dmyCHIMS':
                mapping['O' + d] = '(?P<%s>\\d\\d|\\d| \\d)' % d
            mapping['Ow'] = '(?P<w>\\d)'
        else:
            mapping.update({'Od': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[1:32], 'd', '3[0-1]|[1-2][0-9]|0[1-9]|[1-9]'), 'Om': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[1:13], 'm', '1[0-2]|0[1-9]|[1-9]'), 'Ow': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[:7], 'w', '[0-6]'), 'Oy': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits, 'y', '[0-9][0-9]'), 'OC': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits, 'C', '[0-9][0-9]'), 'OH': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[:24], 'H', '2[0-3]|[0-1][0-9]|[0-9]'), 'OI': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[1:13], 'I', '1[0-2]|0[1-9]|[1-9]'), 'OM': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[:60], 'M', '[0-5][0-9]|[0-9]'), 'OS': self._TimeRE__seqToRE(self.locale_time.LC_alt_digits[:62], 'S', '6[0-1]|[0-5][0-9]|[0-9]')})
    ...
    def _TimeRE__seqToRE(self, to_convert, directive, altregex=None):
