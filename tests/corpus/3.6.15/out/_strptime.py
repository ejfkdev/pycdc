...
class TimeRE(dict):
    ...
    def __init__(self, locale_time=None):
        ...
        base = super()
        base.__init__({'d': '(?P<d>3[0-1]|[1-2]\\d|0[1-9]|[1-9]| [1-9])', 'f': '(?P<f>[0-9]{1,6})', 'H': '(?P<H>2[0-3]|[0-1]\\d|\\d)', 'I': '(?P<I>1[0-2]|0[1-9]|[1-9])', 'G': '(?P<G>\\d\\d\\d\\d)', 'j': '(?P<j>36[0-6]|3[0-5]\\d|[1-2]\\d\\d|0[1-9]\\d|00[1-9]|[1-9]\\d|0[1-9]|[1-9])', 'm': '(?P<m>1[0-2]|0[1-9]|[1-9])', 'M': '(?P<M>[0-5]\\d|\\d)', 'S': '(?P<S>6[0-1]|[0-5]\\d|\\d)', 'U': '(?P<U>5[0-3]|[0-4]\\d|\\d)', 'w': '(?P<w>[0-6])', 'u': '(?P<u>[1-7])', 'V': '(?P<V>5[0-3]|0[1-9]|[1-4]\\d|\\d)', 'y': '(?P<y>\\d\\d)', 'Y': '(?P<Y>\\d\\d\\d\\d)', 'z': '(?P<z>[+-]\\d\\d[0-5]\\d)', 'A': self._TimeRE__seqToRE(self.locale_time.f_weekday, 'A'), 'a': self._TimeRE__seqToRE(self.locale_time.a_weekday, 'a'), 'B': self._TimeRE__seqToRE(self.locale_time.f_month[1:], 'B'), 'b': self._TimeRE__seqToRE(self.locale_time.a_month[1:], 'b'), 'p': self._TimeRE__seqToRE(self.locale_time.am_pm, 'p'), 'Z': self._TimeRE__seqToRE((tz for tz_names in self.locale_time.timezone for tz in tz_names), 'Z'), '%': '%'})
...
def _strptime(data_string, format='%a %b %d %H:%M:%S %Y'):
    ...
    if year is None and iso_year is not None:
        ...
        if julian is not None and week_of_year is None and iso_week is not None:
...
def _strptime_datetime(cls, data_string, format='%a %b %d %H:%M:%S %Y'):
    ...
    if gmtoff is not None:
        tzdelta = None(gmtoff, seconds=datetime_timedelta)
        if tzname:
