    ...
    def _interpolate_some(self, parser, option, accum, rest, section, map, depth):
        ...
        while rest:
            ...
            if c == '{':
                ...
                if '$' in v:
                    None(self._interpolate_some, parser, opt, accum, v, sect(dict(sect, True, raw=parser.items)), depth + 1)
                else:
                    accum.append(v)
                continue
            raise InterpolationSyntaxError(option, section, "'$' must be followed by '$' or '{', found: %r" % (rest,))
...
class RawConfigParser(MutableMapping):
    ...
    OPTCRE = None(re.compile('=|:', delim=_OPT_TMPL.format), re.VERBOSE)
    OPTCRE_NV = None(re.compile('=|:', delim=_OPT_NV_TMPL.format), re.VERBOSE)
    ...
    def __init__(self, defaults=None, dict_type=_default_dict, allow_no_value=False, *, delimiters=('=', ':'), comment_prefixes=('#', ';'), inline_comment_prefixes=None, strict=True, empty_lines_in_values=True, default_section=DEFAULTSECT, interpolation=_UNSET, converters=_UNSET):
        ...
        if delimiters == ('=', ':'):
            ...
            if allow_no_value:
                self._optcre = None(re.compile(d, delim=self._OPT_NV_TMPL.format), re.VERBOSE)
            else:
                self._optcre = None(re.compile(d, delim=self._OPT_TMPL.format), re.VERBOSE)
    ...
    def _validate_value_types(self, *, section='', option='', value=''):
        ...
        if not not self._allow_no_value:
            if value:
