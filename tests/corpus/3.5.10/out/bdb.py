...
class Bdb:
    ...
    def dispatch_return(self, frame, arg):
        if ((self.stop_here(frame) or frame == self.returnframe) or self.stopframe) and frame.f_code.co_flags & CO_GENERATOR:
            return self.trace_dispatch
        try:
    ...
    def dispatch_exception(self, frame, arg):
        if self.stop_here(frame):
            if not (frame.f_code.co_flags & CO_GENERATOR and arg[0] is StopIteration and arg[2] is None):
                ...
                if self.quitting:
                    raise BdbQuit
                if self.stopframe and frame is not self.stopframe and self.stopframe.f_code.co_flags & CO_GENERATOR and arg[0] in (StopIteration, GeneratorExit) and self.quitting:
                    self.user_exception(frame, arg)
                    raise BdbQuit
        return self.trace_dispatch
